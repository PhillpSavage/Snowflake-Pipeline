...
-- give HOL_ROLE to SYSADMIN role;
GRANT ROLE HOL_ROLE TO ROLE SYSADMIN;     
-- 
GRANT ROLE HOL_ROLE TO USER IDENTIFIER($MY_USER);

-- Allow HOL_ROLE to execute Snowflake tasks anywhere in the account
GRANT EXECUTE TASK ON ACCOUNT TO ROLE HOL_ROLE;
GRANT MONITOR EXECUTION ON ACCOUNT TO ROLE HOL_ROLE;
-- user with role HOL_ROLE can query views, history, tables and ... inside snowflake database that is a built-in read-only special database in snowflake and contains query account usage views and other system-level metadata.
GRANT IMPORTED PRIVILEGES ON DATABASE SNOWFLAKE TO ROLE HOL_ROLE;

-- Databases
CREATE OR REPLACE DATABASE HOL_DB;
-- HOL_ROLE becomes the owner of database HOL_DB; ownership means full control over the object
...
-- number(35, 4) 35 is precion that is a total number of digits you can store, 4 is scale that is number of digits that can appear after decimal.
